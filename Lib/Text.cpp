...
void RenderText(const char *text, float x, float y, vec3 color, float scale, mat4 view) {
	...
	VertexAttribPointer(textShaderProgram, "point", 4, GL_FLOAT, GL_FALSE, 4*sizeof(float), 0);
...
void Text(vec3 p, mat4 m, vec3 color, float scale, const char *format, ...) {
    ...
    if (format) {
        va_list ap;
        va_start(ap, format);
        _vsnprintf(buf, 500, format, ap);
        va_end(ap);
    }
	vec2 s = ScreenPoint(p, m);
	RenderText(buf, s.x, s.y, color, scale, ScreenMode());
}
...
void Text(int x, int y, vec3 color, float scale, const char *format, ...) {
	...
	RenderText(buf, (float) x, (float) y, color, scale, ScreenMode());
}
